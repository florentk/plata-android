   ...
   /* private void setNeighbour(String neighbour) {
    ...
    private void alertUpdate(CrossingCMO crossingCMO, Geolocation geolocation){
        ...
        switch(crossingCMO.getDecision()){
            case CrossingCMO.DECISION_NONE:
                ...
                txt.setText(df.format( geolocation.getCurrentSpeed() * 3.6) + " km/h");
                break;
    ...
    private void initMapView(
        ...
        try {
            ...
            final ZipFileArchive zipfFileArchiveValUniv = ZipFileArchive.getZipFileArchive(new File("/sdcard/osmdroid/valenciennes-univ18.zip"));
            final ZipFileArchive zipfFileArchiveVal = ZipFileArchive.getZipFileArchive(new File("/sdcard/osmdroid/valenciennes16.zip"));

            archiveFiles = new IArchiveFile[] { zipfFileArchiveVDA, zipfFileArchiveLille,zipfFileArchiveValUniv,zipfFileArchiveVal};
        } catch (IOException e) {
            e.printStackTrace();
        }
...
*/
    ...
    private Geolocation initGeolocation(LocationManager locationManager) {
        ...
        try {
            ...
            GeolocationTrace geoTrace = GeolocationTraceAndroid.traceFromFile("/sdcard/plaiimob/trace",300,locationManager);
            geoTrace.startTrace();
            geo = geoTrace;
        } catch (IOException e) {
            //default trace
            geo = new GeoLocationAndroid(locationManager);
        }
